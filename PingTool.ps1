...
for ($j = 1; $j -le $watchTime; $j++) {
    ...
    for ($preprocessingId = 0; $preprocessingId -lt $latencyData.Count; $preprocessingId++) {
        ...
        $tempData | ForEach-Object {
            ...
            if ($_["target"] -eq $latencyData[$preprocessingId]["target"]) {
                ...
                #total处理
                if ($latencyData[$preprocessingId]["total"] -eq 0 && $_["last"] -eq 9999) {
                    $latencyData[$preprocessingId]["total"] = 1
                }
                if ($latencyData[$preprocessingId]["hourTotal"] -eq 0 && $_["last"] -eq 9999) {
                    $latencyData[$preprocessingId]["hourTotal"] = 1
                }
                if ($latencyData[$preprocessingId]["todayTotal"] -eq 0 && $_["last"] -eq 9999) {
                    $latencyData[$preprocessingId]["todayTotal"] = 1
                }
                if ($_["last"] -ne 9999) {
                ...
                #lossUseTotal处理
                $latencyData[$preprocessingId]["lossUseTotal"]++
                $latencyData[$preprocessingId]["hourLossUseTotal"]++
                $latencyData[$preprocessingId]["todayLossUseTotal"]++
    ...
    if ($displayMode -eq "latencyList") { 
        ...
        for ($i = 0; $i -lt $domains.Count; $i++) {
            ...
            Write-Host "LOSS："$([Math]::Round($($($latencyData[$i]["connectionFalseCount"]) / $($latencyData[$i]["lossUseTotal"]) * 100), 2))"%" 
            ...
            Write-Host "LOSS："$([Math]::Round($($($latencyData[$i]["todayConnectionFalseCount"]) / $($latencyData[$i]["todayLossUseTotal"]) * 100), 2))"%" 
            ...
            Write-Host "LOSS："$([Math]::Round($($($latencyData[$i]["hourConnectionFalseCount"]) / $($latencyData[$i]["hourLossUseTotal"]) * 100), 2))"%" 
            Write-Host
